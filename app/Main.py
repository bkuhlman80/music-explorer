...
if page == "Overview":
    ...
    try:
        ...
        if not ew.empty and {"a_name", "b_name", "total_weight"}.issubset(ew.columns):
            ...
            a = st.selectbox(
                "Collaborator seed", sorted(set(ew["a_name"]).union(ew["b_name"]))
            )
            mask = (ew["a_name"] == a) | (ew["b_name"] == a)
            sub = ew[mask].copy()
            sub["partner"] = sub.apply(
                lambda r: r["b_name"] if r["a_name"] == a else r["a_name"], axis=1
            )
    ...
    for name in ["artist_discography", "collaboration_edges_weighted", "genre_trends"]:
        try:
            df = load_csv(name)
            st.download_button(
                f"Download {name}.csv", df.to_csv(index=False), file_name=f"{name}.csv"
            )
        except Exception as e:
