...
def _write(df, name, pk=None):
    ...
    if pk: df = df.drop_duplicates(subset=pk, keep="first")
...
def clean_artists(raw):
    ...
    df = pd.DataFrame(rows) if rows else pd.DataFrame(columns=[
        "artist_mbid","name","sort_name","disambiguation","type","gender",
        "area_mbid","begin_date","end_date","ended"
    ])
    # validate PK + FK shapes
    bad = ~df["artist_mbid"].map(is_uuid)
    for _, r in df[bad].iterrows(): _append_reject("bad_artist_mbid","artists",r.to_dict())
    df = df[~bad]
...
def clean_release_groups(raw):
    ...
    df = pd.DataFrame(rows, columns=["rg_mbid","title","primary_type","first_release_date","artist_credit"])
    bad = ~df["rg_mbid"].map(is_uuid)
    for _, r in df[bad].iterrows(): _append_reject("bad_rg_mbid","release_groups",r.to_dict())
    df = df[~bad]
...
def clean_releases(raw):
    ...
    df = pd.DataFrame(rows, columns=["release_mbid","rg_mbid","title","date","country","barcode","status"])
    dfl = pd.DataFrame(rel_labels, columns=["release_mbid","label_mbid","catalog_number"])
    bad = ~df["release_mbid"].map(is_uuid)
    for _, r in df[bad].iterrows(): _append_reject("bad_release_mbid","releases",r.to_dict())
    df = df[~bad]
    _write(df,"releases", pk=["release_mbid"])
    if not dfl.empty:
        dfl = dfl.dropna(subset=["release_mbid","label_mbid"])
        _write(dfl.drop_duplicates(), "release_labels", pk=["release_mbid","label_mbid","catalog_number"])
    else:
        _write(pd.DataFrame(columns=["release_mbid","label_mbid","catalog_number"]), "release_labels")
    return df
...
def clean_recordings_and_tracks(raw):
    ...
    dfr = pd.DataFrame(recs, columns=["recording_mbid","title","length_ms","video"]).drop_duplicates(subset=["recording_mbid"])
    dft = pd.DataFrame(tracks, columns=["track_mbid","release_mbid","recording_mbid","medium_index","position","title","length_ms"])
...
def clean_labels(raw):
    ...
    df = pd.DataFrame(rows, columns=["label_mbid","name","type","disambiguation"]).drop_duplicates(subset=["label_mbid"])
...
def clean_genres_and_tags(raw):
    ...
    def emit(entity_type, entity_id, genres, tags):
        for g in (genres or []):
            grows.append({"entity_type":entity_type,"entity_mbid":entity_id,"genre":g.get("name"),"votes":g.get("count")})
        for t in (tags or []):
            trows.append({"entity_type":entity_type,"entity_mbid":entity_id,"tag":t.get("name"),"votes":t.get("count")})
    for fp, data in raw:
        if isinstance(data, dict) and "release-groups" in data:
            for g in data["release-groups"]:
    ...
    dg = pd.DataFrame(grows, columns=["entity_type","entity_mbid","genre","votes"]).dropna(subset=["entity_mbid","genre"])
    dt = pd.DataFrame(trows, columns=["entity_type","entity_mbid","tag","votes"]).dropna(subset=["entity_mbid","tag"])
    _write(dg.drop_duplicates(), "entity_genres", pk=["entity_type","entity_mbid","genre"])
    _write(dt.drop_duplicates(), "entity_tags", pk=["entity_type","entity_mbid","tag"])
    return dg, dt
