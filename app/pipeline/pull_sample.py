...
def main():
    ...
    ap.add_argument(
        "--base-url", default=os.getenv("MB_BASE_URL", "https://musicbrainz.org/ws/2")
    )
    ...
    ap.add_argument(
        "--rate-limit-ms", type=int, default=int(os.getenv("MB_RATE_LIMIT_MS", "1100"))
    )
    ...
    if not args.user_agent:
        sys.exit(
            "Set USER_AGENT in env or pass --user-agent (format: app/vers (email))"
        )
